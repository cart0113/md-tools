...
This recursively finds all `.md` files under the given folder and reformats
every markdown table to be fixed-width-aligned within 120 characters.
...
Write your own processing function and pass it to the walker:

```python
import md_tools.walker as walker

def my_processor(text, file_path):
    # transform text however you want
    return text.replace("foo", "bar")

walker.walk_and_apply("/my/docs", my_processor)
```
